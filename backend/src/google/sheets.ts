...
export async function ensureHeaders() {
  try {
    ...
    const targetSheetId = SHEETS.target.id;
    if (!targetSheetId) {
      throw new Error('TARGET_SHEET_ID is not configured');
    }
    console.log(`Ensuring headers in target sheet: ${targetSheetId}`);
    // Invoices tab
    await ensureHeaderRow(targetSheetId, SHEETS.target.invoicesTab || 'Invoices', INVOICE_HEADERS, auth);
    console.log(`Headers ensured for ${SHEETS.target.invoicesTab} tab`);
    // Line items tab
    await ensureHeaderRow(targetSheetId, SHEETS.target.linesTab || 'Invoice Line Items', LINE_HEADERS, auth);
